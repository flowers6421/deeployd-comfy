...
class ResourceUsage:
    ...
    def is_within_limits(
        ...
        if self.memory_percent > max_memory_percent:
            return False
        if self.disk_usage_percent > max_disk_percent:
            return False
        return True
...
class ResourceMonitor:
    ...
    def _get_gpu_stats(self) -> Optional[Tuple[float, float, float]]:
    ...
    def check_resource_availability(
        self,
        required_memory_mb: float = 1024,
        required_disk_mb: float = 1024
    ) -> Tuple[bool, str]:
        ...
        if usage.memory_available_mb < required_memory_mb:
            return False, f"Insufficient memory: {usage.memory_available_mb:.0f}MB available, {required_memory_mb:.0f}MB required"
        ...
        if disk_available_mb < required_disk_mb:
            return False, f"Insufficient disk space: {disk_available_mb:.0f}MB available, {required_disk_mb:.0f}MB required"
        
        if usage.disk_usage_percent > self.thresholds["disk_critical"]:
            return False, f"Disk usage too high: {usage.disk_usage_percent:.1f}%"
        
        # Check GPU if needed
        if self.has_gpu and usage.gpu_memory_used_mb is not None:
    ...
    def get_resource_estimate(self, workflow_complexity: Dict[str, Any]) -> Dict[str, float]:
    ...
    def monitor_process(self, pid: int) -> Optional[Dict[str, float]]:
    ...
    def cleanup_old_outputs(self, max_age_hours: int = 24):
        ...
        try:
            ...
            if cleaned_count > 0:
                logger.info(f"Cleaned {cleaned_count} files, freed {cleaned_size / (1024**2):.1f}MB")
