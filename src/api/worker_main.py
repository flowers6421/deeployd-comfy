...
async def display_status(worker_service: WorkerService, refresh_rate: float = 2.0):
    ...
    with Live(layout, console=console, refresh_per_second=1/refresh_rate) as live:
        while not shutdown_event.is_set():
            try:
                ...
                table.add_row("Total Enqueued", str(queue_stats.get("total_enqueued", 0)))
                table.add_row("Total Processed", str(queue_stats.get("total_processed", 0)))
                ...
                table.add_row("Memory Usage", f"{resources.get('memory_percent', 0):.1f}%")
                table.add_row("Memory Available", f"{resources.get('memory_available_mb', 0):.0f} MB")
...
@app.command()
def start(
    comfyui_host: str = typer.Option("localhost", "--host", help="ComfyUI server hostname"),
    comfyui_port: int = typer.Option(8188, "--port", help="ComfyUI server port"),
    workflow_path: str = typer.Option("/app/workflow.json", "--workflow", help="Path to workflow JSON"),
    output_dir: str = typer.Option("/app/outputs", "--output", help="Output directory for images"),
    queue_path: str = typer.Option("/tmp/task_queue.db", "--queue", help="Queue database path"),
    min_workers: int = typer.Option(1, "--min-workers", help="Minimum number of workers"),
    max_workers: int = typer.Option(4, "--max-workers", help="Maximum number of workers"),
    scale_threshold: int = typer.Option(5, "--scale-threshold", help="Queue size per worker before scaling"),
    max_retries: int = typer.Option(3, "--max-retries", help="Maximum task retry attempts"),
    show_dashboard: bool = typer.Option(True, "--dashboard/--no-dashboard", help="Show live status dashboard"),
...
def status(
    ...
    try:
        ...
        table.add_row("Dead Letter Queue Size", str(queue_manager.get_dead_letter_queue_size()))
...
def health(
    ...
    async def check_health():
        ...
        if usage.gpu_memory_used_mb is not None:
