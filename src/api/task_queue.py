...
class TaskQueueManager:
    ...
    def __init__(
        self,
        queue_path: str = "/tmp/comfyui_queue",
    ...
    def fail_task(self, task_id: str, error: str, retry: bool = True):
        ...
        with self.task_lock:
            ...
            if retry and task.retry_count < task.max_retries:
                ...
                logger.info(f"Retrying task {task_id} in {backoff_time}s (attempt {task.retry_count}/{task.max_retries})")
    ...
    def recover_dead_letters(self, max_recover: int = 10) -> List[Task]:
        ...
        for _ in range(min(max_recover, self.dead_letter_queue.size)):
            try:
                ...
                if "Empty" in str(e.__class__.__name__) or self.dead_letter_queue.size == 0:
                    break
        ...
        with self.task_lock:
            for task_id, task in self.active_tasks.items():
                if task.status == TaskStatus.COMPLETED:
                    if task.completed_at and (current_time - task.completed_at) > older_than_seconds:
    ...
    def _get_queue_for_priority(self, priority: TaskPriority):
    ...
    def _get_queues_for_priority(self, priority: TaskPriority):
    ...
    def close(self):
        """Close all queues."""
        for queue in [self.high_queue, self.normal_queue, self.low_queue, self.dead_letter_queue]:
            try:
                queue.close()
            except Exception as e:
