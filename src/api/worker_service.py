...
from src.api.task_queue import TaskQueueManager, Task, TaskStatus
    ...
    async def monitor_and_scale(self):
        ...
        while self.running:
            try:
                ...
                if (queue_size > self.scale_threshold * active_workers and
                    active_workers < self.max_workers and
                    usage.memory_percent < 70 and
                    usage.cpu_percent < 80):
                    
                    await self.add_worker()
                    logger.info(f"Scaled up to {len(self.workers)} workers (queue size: {queue_size})")
                
                # Scale down if queue is small
                elif (queue_size < active_workers and
