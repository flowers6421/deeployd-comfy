...
from src.api.task_queue import Task, TaskStatus, TaskQueueManager
...
class TaskExecutor:
    ...
    def __init__(
        ...
        self.progress_tracker = ProgressTracker(websocket_manager) if websocket_manager else None
        ...
        # Execution tracking
        self.active_tasks = {}
        self.task_resources = {}  # Track resource usage per task
        ...
        try:
            ...
            if self.progress_tracker:
                await self.progress_tracker.start_execution(
                    task.prompt_id,
                    total_steps=task.parameters.get("steps", 20)
                )
    ...
    async def check_resources(self, task: Task) -> tuple[bool, str]:
        ...
        can_execute, reason = self.resource_monitor.check_resource_availability(
            ...
            required_disk_mb=required_disk
        )
    ...
    async def monitor_task_resources(self, task_id: str):
                        ...
                        await self.websocket_manager.broadcast_to_prompt(
                            task.prompt_id,
                            {
                                ...
                                "gpu_percent": (usage.gpu_memory_used_mb / usage.gpu_memory_total_mb * 100)
                                              if usage.gpu_memory_total_mb else None
                            }
                        )
        ...
        if workflow:
            for node in workflow.values():
                if isinstance(node, dict):
                    ...
                    if class_type and not any(x in class_type for x in 
                                             ["Load", "Save", "Sampler", "VAE", "CLIP"]):
                        custom_nodes += 1
    ...
    async def execute_with_callback(
        ...
        Returns:
            Execution result
        """
        # Wrap the workflow executor's progress handling
        async def wrapped_callback(node_id: str, progress: float):
